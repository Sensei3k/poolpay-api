...
pub fn ocr_pdf(pdf_path: &Path) -> Result<String, Box<dyn std::error::Error>> {
    ...
    // Remove the per-page JPEGs generated by pdftoppm — the caller is responsible
    // for deleting the original PDF; these intermediates are ours to clean up.
    for path in &page_files {
        if let Err(e) = std::fs::remove_file(path) {
            error!(path = %path.display(), error = %e, "Failed to clean up PDF page file");
        }
    }
