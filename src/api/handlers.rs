...
pub async fn get_receipts(
    ...
    // is therefore monotonic with arrival order. Legacy rows whose
    // `ingested_at` is still NONE fall back to `created_at` (also
    // server-stamped at insert) so they sort deterministically alongside
    // freshly stamped rows instead of clustering unpredictably.
    let sql = format!(
        "SELECT count() FROM receipt WHERE {where_clause} GROUP ALL; \
         SELECT * FROM receipt WHERE {where_clause} \
         ORDER BY (IF ingested_at IS NONE THEN created_at ELSE ingested_at END) ASC, id ASC \
