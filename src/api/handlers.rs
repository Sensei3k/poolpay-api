...
pub async fn get_receipts(
    ...
    // by the boot-time backfill in `define_receipt_extensions` — by the
    // time this handler runs, every row has `ingested_at` populated, so
    // we don't need a query-time fallback (and SurrealQL doesn't expose
    // one that's valid inside ORDER BY anyway — `IF/THEN/ELSE/END` is a
    // statement, not an expression, and the `??` operator is rejected
    // here too).
    let sql = format!(
