...
pub async fn get_receipts(
    ...
    // Order by the server-stamped ingested_at, not the bot-supplied
    // received_at. A misclocked or hostile bot otherwise controls queue
    // position; ingested_at is generated by `server_now()` at insert and
    // is therefore monotonic with arrival order. Legacy rows are handled
    // by the boot-time `backfill_receipt_ingested_at` in `db.rs`, which
    // runs after the migration runner and covers everything written
    // before this binary booted. Rows written
    // by an older binary still mid-rolling-deploy can land here with
    // `ingested_at = NONE` — those sort first under SurrealDB's NONE-low
    // ordering and get backfilled on the next restart, so the window is
    // self-healing rather than permanently visible. We accept that
    // bounded skew instead of adding a query-time fallback because
    // SurrealQL doesn't expose one that's valid inside ORDER BY —
    // `IF/THEN/ELSE/END` is a statement, not an expression, and the
    // `??` operator is rejected here too.
    let sql = format!(
    ...
    let payment_date = chrono::DateTime::parse_from_rfc3339(payment_source)
        ...
        .map_err(|_| {
            AppError::Conflict(format!(
                "receipt {id} has an invalid {source_field} timestamp"
            ))
        })?;
